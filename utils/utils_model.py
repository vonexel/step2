...
def getCi(accLog):
    """
    Calculates the mean and 95% confidence interval from an accuracy log.
    """
    mean = np.mean(accLog)
    std = np.std(accLog)
...
def get_logger(out_dir):
    """
    Sets up a logger for the project that outputs to both a file and stdout.
    """
...
def initial_optim(decay_option, lr, weight_decay, net, optimizer):
    """
    Initializes an optimizer (Adam or AdamW) with specified parameters and decay options.
    """
...
def get_motion_with_trans(motion, velocity) : 
    """
    Processes motion data with translation/velocity information for visualization purposes.
